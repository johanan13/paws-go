...
const petSchema = new mongoose.Schema({
  petId: { type: String, required: true, unique: true }, // Add petId field
...
app.post('/api/pets', upload.single('petPhoto'), async (req, res) => {
  try {
    ...
    const petCount = await Pet.countDocuments();  // Count existing pets to generate a new ID
    const petId = `PET-${String(petCount + 1).padStart(3, '0')}`;  // Format: PET-001, PET-002, etc.
    ...
    const newPet = new Pet({
      petId, // Add petId to the pet document
      ownerId,
      name,
      species,
      breed,
